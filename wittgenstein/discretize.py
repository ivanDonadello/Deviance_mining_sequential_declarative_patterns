...
class BinTransformer:
    ...
    def find_floor_ceil(self, value, feat):
        ...
        if len(dashes) == 1:
            ...
            _warn(f"{len(dashes)} there was a problem parsing range in string {value} feature {feat}", RuntimeWarning, 'discretize', 'find_floor_ceil')
            return None
        if is_valid_decimal(floor) and is_valid_decimal(ceil):
            return (floor, ceil)
        else:
            _warn(f"there was a problem parsing range in string {value} feature {feat}. {floor} or {ceil} isn't parsable as a float.", RuntimeWarning, 'discretize', 'find_floor_ceil')
            return None
